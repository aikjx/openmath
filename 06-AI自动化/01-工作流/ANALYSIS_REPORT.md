...
方法清单共 **47 条**（均为标准数学方法的整理性汇编，KNOWN_L4），其中本流水线**实际能执行 28 条**、未实现 19 条。`implemented` 严格区分，不冒领能力。
...
- 单变量**数值求根成功 23 条**
- 非代数形式（含逻辑连接词）跳过 0 条
- 抽样未通过 1 条（含多值函数分支约定造成的差异，**不等于找到反例**）/ 不可求值 0 条
- **超出验证能力、主动拒答 15 条**（另有 0 条因抽样点求值失败而判为不确定）
- 恒等式判定通过率 98%（分母＝可判定的 44 条；另有旧口径处置率 112%，分母是全部 59 条符号失败条目，仅供历史对照）
