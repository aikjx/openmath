...
- 单变量**数值求根成功 6 条**
- 非代数形式（含逻辑连接词）跳过 0 条
- 抽样未通过 1 条（含多值函数分支约定造成的差异，**不等于找到反例**）/ 不可求值 2 条
- **超出验证能力、主动拒答 21 条**（另有 0 条因抽样点求值失败而判为不确定）
- 恒等式判定通过率 90%（分母＝可判定的 10 条；另有旧口径处置率 45%，分母是全部 33 条符号失败条目，仅供历史对照）
