...
| `audit.py` | **独立审计**：用**与被测代码不同的算法**重算本仓库自己算出的每个数字（算术函数/π(x)/li/划分/图/群/同调/验证器判准率与拒答率） | 只报差异，不修改被审对象 |
...
- `numeric.verify_identity` **只对能力范围内的初等代数恒等式给出 holds/fails**。
  超出范围的（含级数/积分/微分/集合/量词、变量表不可信、存在隐式乘法、
  实为待解方程）一律返回 `not_decidable` 并拒答。
  理由：那类结论反映的是求值器的局限，不是式子的数学性质
  （2026-09-19 审计曾在此处发现 14 条假阴性）。
- `audit.py` 通过**不等于**结论正确：它只说明在给定对象集与给定独立算法下
  没有发现差异。审计的参考值本身也可能出错（本轮就发生过一次，
  已记录在工作流 `openmath_audit.py` 的 `RECORDED_FIXES` 中）。
